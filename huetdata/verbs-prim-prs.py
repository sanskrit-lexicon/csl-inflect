...
"""verbs-prim-prs.py
   ...
   Handle only the 'v' forms where 
    -- cj is 'prim' and 
    -- 'sys' is 'prs' or 'pef'
   Modified from ~/Documents/GitHub/elispsanskrit/huetcompare/verbs-prim-prs
     Python3 compatible.
...
Huet_padas_prt = {'atma':'A','para':'P','pass':'Q'}  
...
class Huet_verb_prim_prs(object):
 ...
 def update_form(self,form,theclass,pada,tense,person,number):
  ...
  if tense not in self.tenses[cp]:
   ...
   self.tables[(theclass,pada,tense)] = [[] for i in range(0,9)]
...
def main(filein,fileout,fileout1,htense):
 """ Use lxml """
 recs=[]
 ...
 for _, element in etree.iterparse(filein, tag='f'):
  ...
  for  inflected_form in inflected_forms:
   ...
   [syselt] = list(sys)
   #if syselt.tag = 'prs':
   if syselt.tag not in ['prs','pas','pef']:  # present or passive or per. fut.
    continue
   # for htense = 'prs', allow syselt.tag to be 'prs' or 'pas'
   # for other values of htense, require syselt.tag to be same as htense.
   if htense in ['pef']:
    if syselt.tag != htense:
     continue
   if syselt.tag == 'prs':
    theclass = syselt.get('gn')
    assert (theclass in [("%s"%i) for i in range(0,12)]),"Unexpected gn=%s\n" %theclass
    ...
    assert (pada in ["para","atma"]),"Unexpected pada tag=%s\n" % pada
    pada = Huet_padas_prt[pada]
    tense = syselt.tag # pef
    theclass = '_' # placeholder
   else:  # syselt.tag == 'pas' (passive)
    # there is no padaelt
    [md] = list(syselt)
    [tenseelt]=list(md)
    tense = tenseelt.tag
 ...
 stemlist.sort(key = lambda x: x.translate(slp_from_to))
 ...
 for stem in stemlist:
  rec = stemdict[stem]
  tabs = rec.tables
  tabkeys = sorted(rec.tables.keys())
  #tabkeys.sort()
...
if __name__ == "__main__":
 filein = sys.argv[1]
 htense = sys.argv[2] # unused
 stemfile = sys.argv[3]
 tabfile = sys.argv[4]
 main(filein,stemfile,tabfile,htense)
