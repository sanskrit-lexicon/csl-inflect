...
"""verbs-tp.py
   08-14-2016, ejf
   Read huet's SL_morph.xml, and generate 
   ...
   04-21-2020 Produce output comparable to that csl-inflect.
   04-24-2020 Now read SL_morph.xml, which contains not only verbs.
"""
...
def extract_data(filein,htense):
 """ Use lxml """
 ...
 return stemlist,stemdict

def write_stemlist(fileout,stemlist):
...
def prettify_one(y):
 ...
 # return as string with '/' separator
 z = '/'.join(y)
 return z
...
def write_tables(fileout,stemlist,stemdict):
...
if __name__ == "__main__":
 ...
 stemlist,stemdict = extract_data(filein,htense)

 #fileout = "huet_stems_%s.txt" % htense
 write_stemlist(filestem,stemlist)

 #fileout1 = "huet_conj_tables_%s.txt" % htense
 write_tables(filetab,stemlist,stemdict)
