...
"""test2.py Begun May 14, 2016
 ...
 Nove 2019, converted to Python3 form.
Tracing info:
...
;   The following classes of roots are 'sew' (insert 'i'):
...
;     The following classes of roots are 'aniw' (do not insert 'i'):
...
; in that they are 'aniw' (do not insert 'i')
; (3) Third general rule.
;     The following roots are 'vew' (optionally insert 'i'):
;  NOTE: Kale has a much longer list (see below)
; Based on Kale (p. 296-7), the following roots are 'vew'
...
;  vah:  from aniw to vew (ref Whitney, example Scharf)
; shI: removed from 'sew-exceptions', put in 'vew-exceptions' (Whitney)
 """
...
;    (b) changing the ending 'ay' to 'y'
 ...
 if root in ['Kan','jan','tan','san']:
  ...
  if root.endswith('A') and isinstance(base,str) and base.endswith('e'):
   base = base[0:-1]+'A'
 if not isinstance(base,list):
  # make it a list
  base = [base]
 #print('test2: line 1256: base=',base)
 try:
  ans = list(map(lambda b: b+'y',base))
 except:
  print(err0)
  print('base=',base)
  exit(1)
...
def conjugation_join(base,sup,option=None,dbg=False):
 ...
 if isinstance(sup,list):
  ...
  anslist = list(map(f,sup))
  return ','.join(anslist)
 if isinstance(base,list):
  def f(b):
   return conjugation_join(b,sup,option,dbg)
  anslist = list(map(f,base))
  return ','.join(anslist)
...
def conjugation_tab_2(base,tense,theclass,pada,root,dbg=False):
 ...
 for i in range(0,n):  # Recall n is number of endings, i.e., 9
...
def future_base(root,theclass,pada,upasargas,tense=None,sew_gen=None,dbg=False):
 ...
 if theclass == '11': # causal
  ...
  # Note: As these are roots of more than one syllable, they are 'sew'.
...
def ForC_join(base,sew_code,sup,root,strength,i_insert,dbg=False):
 ...
 base = solution(base)
 if isinstance(sup,list):
  return list(map(lambda x: ForC_join(base,sew_code,x,root,strength,i_insert,dbg=dbg),sup))
 if isinstance(base,list):
  return list(map(lambda x: ForC_join(x,sew_code,sup,root,strength,i_insert,dbg=dbg),base))
 if sew_code == 'vew':
...
def ForC_join1(y,sew_code,ending0,root,strength,i_insert,dbg=False):
 ...
 if (root == 'naS') and (efirst in  cons_nonsemivowel):
  ...
  # is if it is an 'i', presetn because sew-code is 'sew'. Since 
...
def aspirate_last_cons(word):
 ...
 ans = list(word) # break into mutable list
 n = len(ans)
 for i in range(n-1,-1,-1): # traverse letters of word in reverse order
  x = ans[i]
  if x in init.consonant_set:
   y = aspirate(x)
   ans[i] = y
...
def conjugation_tab_ForC(upasargas,theclass,pada,root,voice=None,dbg=False):
 ...
 if (pada == 'passive') or (voice == 'passive'):
  ...
  # (b) inserts 'i'.
  # Since 'han' is an 'aniw', this also must be adjusted
  ...
  # Kale 471.
  # 'aniw' roots with a penultimate 'f' change it to
  ...
  # call '..-main' with aniw and with sew
  ...
  # For logic to work properly, 'vew' must be separated here
  ans1 = conjugation_tab_ForC_main(upasargas,theclass,pada,root,'aniw',dbg=dbg)
  ans2 = conjugation_tab_ForC_main(upasargas,theclass,pada,root,'sew',dbg=dbg)
  ans = join_arrays(ans1,ans2)
  ...
  ans = join_arrays(ans1,ans2)
      ...
      (ForC_sym in ['lfw','lfN']):
  # Kale 483 p. 301
  # 'gam P', 'han', and 'aniw' roots ending in 'Ri'
  ...
  # svRi is 'sew' in the 2nd Fut and Conditional (Kale p.305)
  ...
  # 'nRit' is classed as 'vew'
  # however, from Kale dhAtukosha, its 'luT' (periphrastic future)
  # is 'sew' (inserts 'i'). Also, on p. 306, Kale says 'nRit P' is
  ...
  # Kale p. 304. Roots which are 'vew' in luT, lRit, lRi~N
  ans = conjugation_tab_ForC_main(upasargas,theclass,pada,root,'vew',dbg=dbg)
  ...
  # When the conjugation-10 form is used, the roots are 'sew',
  #  which is the conjugation-10 standard.
  # When the non-conjugation-10-form is used, the roots take
  # the general sew-code associated with the root.
...
def join_arrays(a,b):
 ...
 """
 c = zip(a,b)
 ans = list(map(join_arrays_f,c))
 return ans

def join_arrays_many(a):
 c = zip(*a)
 #ans = list(map(lambda x: solution(list(x))),c
 ans = list(map(join_arrays_f,c))
...
; (if (equal voice 'PASSIVE) (setq pada 'A))
 ...
 for xar in sfxtab: # xar is a list of strings
  #xar1 = flatten(xar)
  #yar = list(map(lambda x: map(lambda b: conjugation_join(b, x),base), xar))
  yar = list(map(lambda x: list(map(lambda b: conjugation_join(b, x),base)), xar))
  yar = flatten(yar)
...
def periphrastic_init1(theclass,pada,root):
 ...
 for x in ctab:
  if not isinstance(x,list):
   x = [x]
  y = list(map(lambda x1: conjugation_join('Am',x1),x))
  #z = flatten(y)
...
def periphrastic_init(pada):
 ...
 ans = list(map(lambda x: flatten(list(x)),anst))
...
def periphrastic_base(root,theclass,pada,dtype=None,dbg=False):
 ...
 if not isinstance(ans,list):
  ans = [ans]
 if ans != ['vid']:
  ans = list(map(lambda a: gunate_final_vowel(a),ans))
...
def conjugation_tab_liw_r(upasargas,theclass,pada,root,voice=None,dbg=False):
 ...
 if root in ['Svi','ve','SranT','granT','damB','svaYj','ad','ci','aj','cakz','vid']:
  ...
  if root == 'Svi':
   ...
   #Kale p.323
   # The roots 'SranT', 'granT', 'damB', and 'svaYj' drop their
   # nasal optionally before the terminations of the Perfect.
   # When the nasal is dropped,
   # 'SranT', 'granT', 'damB' obey #500, even before the strong
   # terminations
   ans.append(conjugation_tab_liw_r_main(upasargas,theclass,pada,root,dbg=dbg))
   tok = root
   tok1 = tok[0:-2]+tok[-1:]
   ...
   #Kale #511, p. 324
   # 'Gas' (1P) is to be optionally substituted for 'ad' in the Perfect
   ans.append(conjugation_tab_liw_r_main(upasargas,theclass,pada,root,dbg=dbg))
   ans.append(conjugation_tab_liw_r_main(upasargas,"1",pada,"Gas",dbg=dbg))
  elif root == 'ci':
   #Kale #514, p. 326
   # 'ci' optionally changed to 'ki' after the reduplicative
  ...
  # (mamArjva mamArjiva mamfjva mamfjiva) and
  # (mamArjma mamArjima mamfjma mamfjima).
 ...
 # ans[5] = ['AnaRQve','AnaSiDve'] # was ['AnaSDve','AnaSiDve']
  ...
  # changed to 'G' or to 'Q' when followed by
...
def perfect_join(base,sew_code,sup,dbg=False):
 ...
 """
 if isinstance(sup,list):
  return list(map(lambda x: perfect_join(base,sew_code,x), sup))
 elif isinstance(base,list):
  return list(map(lambda x:  perfect_join(b,sew_code,sup),base))
 elif sew_code == 'vew':
  return list(map(lambda x: perfect_join(base,x,sup),['sew','aniw']))
 else:
...
def reduplicate_perfect(root,wparts,dbg=False):
 ...
 if tok in ['vac','vad','vap','vas','vah','vaS']:
  #Antoine2#114 The roots 'vac' (to speak), 'vad' (to speak),
  #  'vap' (to sow), 'vas' (to dwell), and 'vah' (to carry) reduplicate in
  #  'u' (e.g. 'uvac') Furthermore, before weak terminations, the radical
  ...
  #Antoine2#116. The roots 'vyaD' (to pierce), 'svap' (to sleep),
  # and 'grah' (to seize) reduplicate as follows:
  # 'vivyaD', 'suzvap', 'jagrah'.  Before weak terminations, the
  #radical 'y', 'v' and 'r' are changed to 'i', 'u', and 'f'
  #respectively.  The change of 'y' to 'i', 'v' to 'u', and 'r' to 'f'
  #is called 'samprasAraRa'
  ans = ['vivyaD','viviD']
 elif tok == 'vyac':
  # Kale #506, p. 317
  ans = ['vivyac','vivic']
  ...
  # Kale 524, p. 329
  # 'vyaT' (1 A 'to suffer') takes samprasAraNa in the reduplicative
  ...
  if tok in ['jan','gam','han','Kan','Gas']:
   #Antoine2#117. The roots 'jan' (to be born), 'Kan' (to dig),
   #'gam' (to go), 'Gas' (to eat) and 'han' (to kill) drop their
   #medial 'a' before weak terminations.
   #The 'h' of 'han' is changed to 'G' (in weak and strong forms).
   #The 'j' of 'jan' is changed to 'jY' (in weak form)
   ans1 = ans[0]
   if tok == 'han':
...
def construct_sewPERF_code1a(root,theclass,pada,upasargas,dbg=False):
 """ Unlike 'reduplicate', this returns a list, possibly with just one
     element.
; it is assumed that the 8 exceptional roots (kf ... Sru) are
...
; the terminations 'va ma Ta se vahe mahe Dve'
; (a) All roots, whether 'sew' or 'aniw', admit 'i' except:
;     kf sf Bf vf
;     stu dru sru Sru
;  The eight roots just mentioned are aniw. 
;  In addition to this general rule, there are some modifications
;  applicable only to the ending 'Ta'.
;  Before 'Ta':
;      the root 'kf' (only with prefix 'sam') admits 'i'.
;      the root 'vf' admits 'i'.
;      aniw roots ending in 'f', except the root 'f', reject 'i'
;      aniw roots with a final vowel other that 'f' admit 'i' optionally
;      aniw roots with a penultimate 'a' admit 'i' optionally
;      the root 'f' admits 'i' necessarily (Kale #515)
;kf : aniw sew sam
; the case  'sam-kRi' is handled in function 'conjugation-tab-liT'
; ADDENDA: 
; 1.  based on example on p. 314,
;  the aniw root'sRij' admits 'i' optionally before 
; 'T' (otherwise, it would be required)
; Also, it is treated as sew before other consonants
; 2. Kale p. 318
; In the perfect, 'Svi' is optionally considered as 'Su'.
; 'Svi' is considered 'sew' before 'Ta' and other consonants.
; Based on the table in Kale, this is also the way 'Su' is
; considered.  Otherwise, 'Su' was 'sew vew' for perfect
; Kale #508, p. 319. The roots 'svf', 'sU' amd 'dhU' admit
; 'i' necessarily before consonantal terminations except 'Ta';
; before 'tha' they admit 'i' optionally.
; Note: Kale has these roots mentioned in the general category
; of 'vew' roots. However, in my general classification of sew-code,
; 'dhU' and 'sU' show as 'sew', and 'svf' shows as 'aniw'
; Kale #508, p. 319
; My algorithm gives different results for the following verbs. 
; So I explicitly state the answer (based on Kale)
; All these are classified as general 'vew' by Kale, but this
...
def construct_conjtab1a_spcltense(root,theclass,pada,upasargas,tense,voice,dbg=False):
 ...
 noans = 'N/a'
 ctabs = []
 #print('test2.py. line 5733: bases=',bases)
 for base in bases:
...
Kale 537.
  ...
  Kale 543.
   ...
   They also belong to the 4th variety if they are 'aniw' or
   the 5th variety if they are 'sew' (see aorist-2-543-P)
  Kale 544.
   ...
   according as they are 'aniw' or 'sew' (see aorist-2-544-P)
 Notes.
...
Kale 564. There are both P and A forms
 Kale 565.
  a1. aniw roots not belonging to any of the preceding varieties
    (i.e. 1,2,3,6,7) take this form 
  a2. aniw roots that optionally take any of the preceding varieties
    take this form.
    NOTE: aniw roots in variety '6' generally don't take this variety
  ...
  Exceptions:
   ...
   4. Of 'sew' roots, 'vRi' and those ending in 'RI' when A take 4th or 5th.
...
-----------
 ...
 Kale 561. 
  - 'aniw' roots ending in 'sh Sh s h' and
...
Kale 597(a)
  ...
  Kale 597(c-1)
   ...
   - roots which are 'sew' and end in 'am' are unchanged, in general.
     However, the following 'sew' roots ending in 'am' do take vrddhi:
...
def aorist_4_P(root,theclass,pada,upasargas,dbg=False):
 ...
 if root in ['ric','vic','nij','yuj','vij','kzud','Cid',
  ...
  # Kale#542. This aniw roots take 2nd and 4th varieties
  return True
...
def conjugation_tab_aorist3_main(upasargas,theclass,pada,rootx,sew_code=None,i_insert=None,dbg=False):
 ...
 if [root,theclass,pada] == ['sAmaya','10','p']:
  btab = list(map(lambda x: x.replace('samaya','samayA'),btab))
  print(err0,"KLUDGE")
 return btab
...
def aorist_join(base,sew_code,sup,root,strength,i_insert,dbg=False):
 ...
 if isinstance(sup,list):
  return list(map(lambda x: aorist_join(base,sew_code,x,root,strength,i_insert),sup))
 if isinstance(base,list):
  return list(map(lambda x: aorist_join(x,sew_code,sup,root,strength,i_insert),base))
 if (sew_code == 'vew'):
  return list(map(lambda x: aorist_join(base,x,sup,root,strength,i_insert),['sew','aniw']))
...
def causal_conjtab1a_bases_a(root,theclass,pada,upasargas,tense,voice=None,dbg=False):
 ...
 if tense in ['laN','lfN']:
  ...
  bases = list(map(lambda x: 'a'+x,bases))
 return bases
...
def causal_bases_gentense(root,theclass,pada,upasargas,tense,voice,dbg=False):
 ...
 if (lc in init.vowel_set) and (tense == 'luN3'):
  ...
  bases = list(map(lambda x: 'a'+x,bases0))
...
def causal_base1b(root,theclass,pada,upasargas,voice,dbg=False):
 ...
 if voice == 'passive':
  bases = list(map(lambda x: x[0:-2] + 'y',bases))
 return bases
...
def construct_ippart1a_tvA(root,theclass,pada,upasargas,dtype=None,dbg=False):
 ...
 if theclass == '10':
  ...
  #print(err0,"bases=",bases)
  ans = list(map(lambda b: b + 'i' + ending, bases))
 elif dtype == 'c': # causal
  ...
  if root == 'ku':
   ...
   if (fc in init.consonant_set) and (pc in 'iu') and (not (lc in 'yv')):
    ...
    bases = [root,b]
  ending = 'i' + ending
  ans = list(map(lambda b: conjugation_join(b,ending),bases))
 elif sew_tvA == 'vew':
...
def kta_sew_code(root,theclass,pada,upasargas,dtype=None,dbg=False):
 ...
 if dtype == 'c':
  ...
  # motivating example: causal of 'sthA' should be 'sew'
  ans = 'aniw'
 else:
  ans = sew_code
 return ans
...
def construct_pppart1a_helper(root,theclass,pada,upasargas,sew_kta=None,dtype=None,dbg=False):
 ...
 if theclass == '10':
  ...
  if not isinstance(base,list):
   base = [base]
  base = flatten(base)
  base = list(map(lambda b: b[0:-2],base))
  base = solution(base)
 elif dtype == 'c':
  ...
  if not isinstance(base,list):
   base = [base]
  base = flatten(base)
  base = list(map(lambda b: b[0:-2],base)) # remove the 'ay'
  base = solution(base)
...
def kta_join(base,sew_code,sup,root,dbg=False):
 ...
 """
 if isinstance(sup,list):
  return list(map(lambda x: kta_join(base,sew_code,x,root,dbg=dbg),sup))
 elif isinstance(base,list):
  return list(map(lambda x: kta_join(x,sew_code,sup,root,dbg=dbg),base))
 elif sew_code == 'vew':
  return list(map(lambda x: kta_join(base,x,sup,root,dbg=dbg),['sew','aniw']))
 else:
...
def kta_join1(base,sew_code,sup,root,dbg=False):
 """
 ; based on 'conjugation-join'
 ; sew-code is either nil, 'sew' or 'aniw'
 """
 y = base
 ending0 = sup
 # insert 'i' if needed
...
def declension_pres_part_alt_P(v,g,theclass,pada,dbg=False):
 ...
 def fjoin(e1):
  if not isinstance(e1,list):
   e1 = e1.split(',')
  y = list(map(lambda e: weak+e,e1))
  z = solution(y)
  return z
 ans = list(map(fjoin,endings))
 return ans
...
def declension_pass_part_alt(x,g,theclass,pada,dbg=False):
 ...
 def fjoin(e1):
  if not isinstance(e1,list):
   e1 = e1.split(',')
  #y = list(map(lambda e: weak+e,e1))
  y = list(map(lambda e: declension_join(weak,e),e1))
  z = solution(y)
  return z
 ans = list(map(fjoin,endings))
 return ans
